...
$updateHistory = mysqli_query($con, "UPDATE appointment_history SET ah_status='Accepted' WHERE ah_id={$ah_id}");
$updateAppointments = mysqli_query($con, "UPDATE appointments SET a_status='Accepted' WHERE a_id={$a_id}");
...
if ($updateHistory && $updateAppointments) {
    if ($userId > 0) {
        notificationCreateForUser(
            ...
            "Your appointment #{$a_id} has been accepted for {$appDate} at {$appTime}.",
    ...
    notificationCreateForAllAdmins(
        ...
        "{$clientName}'s appointment #{$a_id} was accepted.",
...
header('Location: appointments_manage.php?toast=error&msg=Failed+to+confirm+appointment.');
exit();
